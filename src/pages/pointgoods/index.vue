  ...
  <div class="goods" v-if="point">
    <div class="goods-wrp">
      <div class="good-banner">
        ...
        >
          <swiper-item
            ...
            class="swiper-item"
          >
            <img :src="item" mode="scaleToFill" @click="previewImage(index)">
          </swiper-item>
        </swiper>
        <div class="views">浏览: {{skudetail.views}}</div>
      ...
      <button open-type="share" class="share">
        <img src="http://p2.icaixiaochu.com/share.png" alt>
      </button>
      <div class="sku-ared">
          ...
          <div class="sku-ared-box">
            ...
            <div class="sku-head-info">
              <div class="sku-info-left">
                <p style="font-size:13px;color:#B2B2B2;text-decoration: line-through;">{{skudetail.sku[0].normal_price}}</p>
                <div class="dash-price">
                  <p class="discont-price" >¥{{skudetail.sku[0].market_price}}</p>
      ...
      <div class="sku-detail">
        <wxParse :content="skudetail.detail" @preview="preview" @navigate="navigate"/>
      </div>
    ...
    <div class="good-card">
      ...
      <div class="card-right" v-if="skudetail.stock >=1">
        ...
        <div class="add-card" @click.stop="addCard">加入购物车</div>
...
export default {
  ...
  methods: {
    ...
    previewImage (index) {
      let imgs = []
      this.skudetail.banner.forEach(element => {
        imgs.push(element.imgurl)
      })
      wx.previewImage({
        // 当前显示下表
        current: index,
    ...
    async eventDraw () {
      let qrcode = await this.$http.post('/getGoodsQrcode', {goods_id: this.good_id})
    ...
    addCard () {
      ...
      let data = this.skudetail
      data.sku = this.skudetail.sku[0]
      this.create_db(data)
    },
    buy () {
    ...
    createOrder () {
      ...
      let goods = {
        ...
        number: this.number,
        real_price: this.skudetail.sku[0].market_price,
