<template>
  <div class="refunded" v-if="refundedData">
    <div class="notification" :style="{ background: activeColor }">
      <div class="arrow" :style="{  borderTop: arrowColor }"></div>
    ...
    <div class="refunded-wrp">
      <div class="refunded-head">
        <div class="refunded-col">
          <p>退款金额</p>
          <span>¥{{refundedData.refund.apply_money}}</span>
        </div>
        <div class="refunded-col">
          <p>退款账户</p>
          <span style="color:#FF6304;">原支付账户</span>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    refundDes () {
      ...
      return this.refundedData.refund.refund_status === 1 ? '已将退款申请提交至原支付账户' : this.refundedData.refund.refund_status === 2 ? '商家拒绝了您的退款申请，可联系商家进一步处理' : '订单正常发货'
    },
    refundTitle () {
      if (!this.refundedData) {
        return ''
      }
      return this.refundedData.refund.refund_status === 1 ? '商家同意退款' : this.refundedData.refund.refund_status === 2 ? '商家拒绝退款' : '自主撤销退款申请'
    }
  },
  components: { popup },
  mounted () {
