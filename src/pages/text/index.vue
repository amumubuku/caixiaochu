...
export default {
      ...
      wx.login({
        success (res) {
          if (res.code) {
                      ...
                      wx.request({
                        url: 'http://localhost:1334/public/pay', // 仅为示例，并非真实的接口地址
                        header: {
                          'content-type': 'application/json' // 默认值
                        },
                        data: {
  ...
  mounted () {
    ...
    //   this.$http.get('/login', {encryptedData: res.encryptedData, iv: res.iv, code: this.code}).then(res => {
    //     console.log(res)
    //   })
    // })
  }
