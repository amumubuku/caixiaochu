<template>
  <div class="my-address">
    <div class="address-wrp">
      <div class="address-item" v-for="(item, index) in address" :key="index" @click="selectAddress(item)">
        <div class="address-info">
          <!-- :style="{backgroundColor: item.position.backgroundColor, color: item.position.textColor}" -->
          <div class="address-position"  :class="[item.label ==0? 'small-2': item.label ==1?'small-3' :item.label == 2? 'small-1': 'small-4']" >{{item.label ==0?'住宿':item.label==1? '学校':item.label==2?'公司':'其他'}}</div>
    ...
    <div class="null-address" v-if="!address.length >= 1">
      <img src="https://img.icaixiaochu.com/42stdFgUrrxlKDKE" alt="">
...
export default {
  ...
  methods: {
    ...
    edit (item) {
      ...
      wx.navigateTo({
        url: `../editaddress/main?id=${item.id}`
      })
    },
